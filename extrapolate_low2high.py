...
def main():
  ...
  for ac in ac_list:
      ...
      init_idx = list(range(n_start,(n_start+n_step))) 
      ...
      top_thpt_ex = rp.loc[idx_nz_ex, ac]

      ###################### 2. GIL+ ######################
      pool_idx_hi = range(U_sub.shape[0])
      seed_idx_hi = []
      ...
      top_thpt_hi = rp.loc[idx_nz_hi, ac]
      ...
      top_thpt_dac = rp.loc[idx_nz_dac, ac]           
      ...
      pred_nn_list = []
      ANN = MLPRegressor(random_state=0, hidden_layer_sizes=16, max_iter=1000)
      ...
      top_thpt_nn = rp.loc[idx_nz_nn, ac]        
      ...
      top_thpt_bo = rp.loc[idx_nz_bo, ac]             

      ###################### 6. Random sampling with linear regression ######################
      seed_idx_rnd = list(range(n_step*n_iter//1))
      ...
      top_thpt_rnd = rp.loc[idx_nz_rnd, ac]                

      ###################### 7. True optimal ######################
      top_thpt_opt = rp.loc[sort_idx_full[-1], ac]

      ###################### 8. Default ######################
      top_thpt_de = rp.loc[sort_idx_full[n_start], ac]
  ...
  np_res = np.column_stack((res[ac1],res[ac2],res[ac3]))
  df = pd.DataFrame(np_res, index= methods, columns=ac_list)
  ...
  df.loc['n-gil+',:]=(df.loc['opt',:] - df.loc['gil+',:])/df.loc['opt',:]
  # df.to_csv('output/low2high/{}.csv'.format(workload), index=True)
  print(df)
